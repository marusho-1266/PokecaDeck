...
HPとタイプ情報は、ワザセクションの前にある要素から取得できる。
...
const extractBasicInfo = async (page) => {
  return await page.evaluate(() => {
    ...
    // タイプ情報（たね/基本/進化）の抽出
    const typeElements = document.querySelectorAll('[class*="icon"], [class*="type"]');
    let cardType = null;
    const bodyText = document.body.innerText;
    if (bodyText.includes('たね')) cardType = 'たね';
    else if (bodyText.includes('基本')) cardType = '基本';
    else if (bodyText.includes('進化')) cardType = '進化';
